...
while true
    ...
    H2=partialehess_tensor(x,1:N);
    g2=getPartialEuclideanGradient(problem, x,1:N);
   
    
    problem.hess=@problem_hess_det;
    ...
    stats = savestats(problem, x, storedb, key, options, k, fx, ...
                      ...
                      rhoden, accept, numit, norm_eta, used_cauchy,elap_time);
    info(k+1) = stats;
...
function h = problem_hess_det(x, u)
...
function stats = savestats(problem, x, storedb, key, options, k, fx, ...
                           norm_grad, Delta, ticstart, info, rho, rhonum, ...
                           rhoden, accept, numit, norm_eta, used_cauchy,elap_time,test_result)
    stats.iter = k;
    stats.cost = fx;
